...
const App = () => {
  ...
  useEffect(() => {
    try {
      ...
      setParseError(err.message || 'Erro ao inicializar conexão com o banco de dados.');
      return;
    }

    // Load public data immediately
    ParseService.fetchPublicData()
      .then(publicData => {
        setData(prev => ({
          ...prev,
      ...
      .catch((err: any) => {
        console.error('[Back4App] Falha ao carregar dados públicos:', err);
        setParseError(`Falha ao conectar com o banco de dados: ${err.message || err}. Verifique suas credenciais e as permissões (CLP) no Back4App.`);
      });
  ...
  const handleUpdateStudentProgress = useCallback(async (studentId: string, stars: number) => {
    if (userRole !== 'admin' && userRole !== 'professor') return;
    try {
        ...
        <form onSubmit={handleSaveTeam} className="space-y-6">
          ...
          <div className="bg-gray-50 p-4 rounded-lg border border-gray-200 space-y-4">
            ...
            <div>
              ...
              <input
                type="email"
                className="w-full rounded-lg border-gray-300 border p-2.5 bg-white text-gray-900 focus:ring-2 focus:ring-jiu-primary focus:border-transparent outline-none transition-all"
