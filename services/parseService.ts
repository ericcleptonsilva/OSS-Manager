...
export const saveStudent = async (studentData: Partial<Student>) => {
  ...
  // Senhas agora são gerenciadas nativamente via Parse.User e não em texto simples no objeto Student.
...
export const saveAcademy = async (academyData: Partial<Academy>) => {
  ...
  // adminPassword não é mais salvo em texto simples por segurança.
    ...
    try {
      ...
      if (!role) {
        try {
          ...
          if (roles.length > 0) {
            const roleName = roles[0].get('name').toLowerCase();
            if (roleName.includes('admin')) role = 'admin';
            else if (roleName.includes('prof')) role = 'professor';
            else role = 'student';
          }
        } catch {
          role = 'student';
      ...
      // Admin padrão consolidated: fallback para VITE_INITIAL_ADMIN_EMAIL / VITE_INITIAL_ADMIN_PASSWORD
      if (team) {
        const fallbackEmail = import.meta.env.VITE_INITIAL_ADMIN_EMAIL || '';
        const fallbackPass = import.meta.env.VITE_INITIAL_ADMIN_PASSWORD || '';
        const storedAdminEmail = (team.get('adminEmail') || fallbackEmail).trim().toLowerCase();
        const storedAdminPass = team.get('adminPassword') || fallbackPass;
        
        // Ensure explicit rejection of empty string credentials to prevent bypass
...
export const saveProfessorAccount = async (email: string, name: string, password?: string): Promise<Parse.Object> => {
  ...
  // A senha não deve ser salva aqui em texto simples.
